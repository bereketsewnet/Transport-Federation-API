// src/migrations/run-terminated-unions-location-fields-migration.js
// Run this script to add location fields (region, zone, city, sub_city, woreda, location_area) to terminated_unions table
...
async function runMigration() {
  try {
    ...
    console.log('\nAdding location fields to terminated_unions table...');
    ...
    const [results] = await sequelize.query(`
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'terminated_unions' 
    ...
    // Add columns one by one after external_audit_date
    if (!existingColumns.includes('region')) {
      await sequelize.query(`
        ALTER TABLE terminated_unions 
